...
| --- | --- | --- |
| `test:doctor` | `pnpm run test:doctor` | Full quality check: static + types + dynamic |
...
| `test:unit` | `pnpm run test:unit` | Run Jest unit tests |
| `test:dynamic` | `pnpm run test:dynamic` | Alias for `test:unit` |
...
| `cleanup` | `pnpm run cleanup` | No-op (no build artifacts) |
